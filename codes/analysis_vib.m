function analysis_vib(healthy_old,faulty_old,feature_old,datawidth_old,datasets_old,maxrejects,P_Ranges)
...
for k=1:100;        %features are  being printed,the best one comes the first
    colm=mod(sorted(k),feature_old);
    press=floor(sorted(k)/feature_old);
    fprintf('feature%d      pressure%d \n',colm,press+1);
end
end
