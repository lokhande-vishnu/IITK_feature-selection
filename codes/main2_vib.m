function main2_vib(healthy_old,faulty_old,feature_old,datawidth_old,dataset_old,P_Ranges)
...
feature=feature_old*P_Ranges;datawidth=datawidth_old/4;dataset_brej=dataset_old;
...
k=rej_func(brej_healthy,brej_faulty,feature,datawidth,dataset_brej,thresh_zcr,thresh_separation,thresh_ratio,thresh_std);
if(k~=0)
...
if(k~=0)
    ...
    healthy(ind+1:(dataset_brej*datawidth)-datawidth,:)=brej_healthy(ind+datawidth+1:(dataset_brej*datawidth),:);
    faulty(ind+1:(dataset_brej*datawidth)-datawidth,:)=brej_faulty(ind+datawidth+1:(dataset_brej*datawidth),:);
    dataset=dataset_brej-1;
else
...
for i=1:feature;
    if(score(ind(i))~=0)
        ...
        p1=2*(pres+1);
        p2=2+2*(pres+1);
        fprintf('feature%d    pressure %d-%d  \n',colm,p1,p2);
        x_plot = 1:(dataset+1)*datawidth;
        h = figure;
        plot(x_plot, brej_healthy(:,ind(i)), '-g', x_plot, brej_faulty(:,ind(i)), '-b');
        set(gca,'XTick',0:datawidth:(dataset+1)*datawidth);
        grid on;
        
        path = sprintf('F:\\Desktop\\result\\Fea_%d',ind(i));
        legend({'Healthy','Faulty'}, 'Location', 'NorthEast');
        ti = sprintf('Fea%d  press %d-%d',colm,p1,p2);
        title(ti);
